# docs/UI_GUIDELINES.md — GUSTI Portfolio UI Contract

This is the single active UI policy for the canonical repository `endgame99/gusti-portfolio-current`.

The document converts `docs/PRD.md` into visual, interaction, responsive, and quality-control rules. It is not permission to invent product facts, redesign unrelated surfaces, or copy another brand.
...
- `CONFIRMED` — explicitly supplied by the owner or required by `docs/PRD.md`.
...
6. Optional material in `docs/references/`.
...
|---|---:|---|
| Global page container | 1800px | 16px mobile / 24px tablet / 32px desktop |
...
|---|---|
| Global shell and state | `src/app/App.tsx` |
| Header controls | `src/components/layout/Header.tsx` |
| Sidebar and mobile navigation | `src/components/layout/Sidebar.tsx` |
| Work feed and cards | `src/components/work/WorkGrid.tsx` |
| Featured proof | `src/components/home/HeroCarousel.tsx` |
| Homepage content modes | `src/components/layout/ContentTabs.tsx` |
| Global tokens and layout utilities | `src/styles/index.css` |
| Product content | `src/content/`, `src/content/projects/details.ts` |
