import React, { useEffect, useState } from 'react';
import { 
  ArrowLeft, 
  Calendar, 
  Sparkles, 
  MessageCircle, 
  Sliders,
  Layers,
  Cpu,
  TrendingUp,
  ChevronRight,
  Play,
  Pause,
  Volume2,
  VolumeX,
  Eye,
  Clock,
  ShieldCheck,
  UserCheck
} from 'lucide-react';
import { Language, WorkItem } from '../types';
import { getProjectDetail } from '../data/projectDetails';
import { WHATSAPP_LINK } from '../data';

interface ProjectDetailProps {
  work: WorkItem;
  lang: Language;
  onBack: () => void;
}

export function ProjectDetail({ work, lang, onBack }: ProjectDetailProps) {
  const detail = getProjectDetail(work.id, work.title, work.tags);
  const [sliderValue, setSliderValue] = useState<number>(50);
  const [followed, setFollowed] = useState<boolean>(false);
  const [videoPlaying, setVideoPlaying] = useState<boolean>(true);
  const [videoMuted, setVideoMuted] = useState<boolean>(true);

  // Auto-scroll to top when project loads
  useEffect(() => {
    window.scrollTo({ top: 0, behavior: 'instant' as any });
  }, [work.id]);

  // Generate dynamic WhatsApp link customized for this project
  const getCustomWhatsAppLink = () => {
    let baseUrl = 'https://wa.me/6200000000000';
    if (WHATSAPP_LINK) {
      const match = WHATSAPP_LINK.match(/https:\/\/wa\.me\/([0-9]+)/);
      if (match && match[1]) {
        baseUrl = `https://wa.me/${match[1]}`;
      }
    }
    const text = encodeURIComponent(
      lang === 'id' 
        ? `Halo Gusti, saya tertarik dengan project "${work.title}" (${work.label}). Bisa bantu saya membuat visual serupa untuk produk saya?`
        : lang === 'cn'
        ? `您好 Gusti，我对您的项目 "${work.title}" (${work.label}) 非常感兴趣。您能帮我的品牌制作类似的视觉资产吗？`
        : `Hello Gusti, I am interested in your project "${work.title}" (${work.label}). Can you help me produce similar visual assets for my product?`
    );
    return `${baseUrl}?text=${text}`;
  };

  const getLabel = (key: string) => {
    const labels: Record<string, Record<Language, string>> = {
      back: {
        id: 'Kembali ke Galeri',
        en: 'Back to Works',
        cn: '返回作品集'
      },
      client: {
        id: 'Klien Brand',
        en: 'Brand Client',
        cn: '合作客户'
      },
      year: {
        id: 'Tahun Rilis',
        en: 'Release Year',
        cn: '发布年度'
      },
      role: {
        id: 'Peran Utama Gusti',
        en: 'Gusti\'s Core Role',
        cn: 'Gusti 核心角色'
      },
      services: {
        id: 'Spesialisasi Terapan',
        en: 'Applied Specialties',
        cn: '应用专业领域'
      },
      overview: {
        id: 'Abstraksi Strategi',
        en: 'Strategic Brief',
        cn: '战略简述'
      },
      direction: {
        id: 'Konsep Kreatif',
        en: 'Creative Direction',
        cn: '创意方向'
      },
      scope: {
        id: 'Lingkup Delivery',
        en: 'Scope of Delivery',
        cn: '交付工作范围'
      },
      interest: {
        id: 'Butuh Visual Sekelas Ini?',
        en: 'Ready for High-End Assets?',
        cn: '需要此等级的商业视觉吗？'
      },
      chat_gusti: {
        id: 'Mulai Scoping Project Beserta Anggaran',
        en: 'Initiate Scoping & Production Quotation',
        cn: '开始项目策划与商业报价讨论'
      }
    };
    return labels[key]?.[lang] || labels[key]?.['en'];
  };

  // Dedicated labels for w1 & w11
  const starmapLabels = {
    beforeLabel: {
      id: 'FOTO SKU MENTAH (SAMPEL FLATLAY KLIEN)',
      en: 'RAW SKU PRODUCT (CLIENT FLATLAY SAMPLE)',
      cn: '原始 SKU 产品（客户平铺样片）'
    },
    afterLabel: {
      id: 'PRODUKSI KREATIF GUSTI. (AI-ASSISTED)',
      en: 'GUSTI. CREATIVE PRODUCTION (AI-ASSISTED)',
      cn: 'GUSTI. 创意制作（AI 辅助）'
    },
    sliderTitle: {
      id: 'METAMORFOSIS SKU INTERAKTIF (GESER)',
      en: 'INTERACTIVE SKU METAMORPHOSIS',
      cn: '互动式 SKU 蜕变对比（滑动）'
    },
    sliderSub: {
      id: 'Bandingkan antara foto flatlay produk mentah milik klien dengan hasil akhir modeling virtual beralur kosmik kami.',
      en: 'Drag the slider to compare client\'s raw product flatlay with our final high-end cosmic-themed virtual model rendering.',
      cn: '拖动滑块比较客户端的原始产品平铺图与我们最终的高端宇宙主题虚拟模特渲染。'
    },
    specTitle: {
      id: 'Cetak Biru Produksi AI (STARMAP Spec)',
      en: 'AI Production Blueprint (STARMAP Spec)',
      cn: 'AI 生产蓝图 (STARMAP 规格)'
    },
    specSub: {
      id: 'Bagaimana Gusti mengamankan akurasi produk fisik sekaligus mensintesis lanskap kosmik bernilai tinggi.',
      en: 'How Gusti preserves absolute physical product integrity while synthesizing high-value cosmic landscapes.',
      cn: 'Gusti 如何在合成高价值宇宙景观的同时，保持绝对的产品物理真实性。'
    }
  };

  const starmapSpecs = [
    {
      icon: <Cpu className="w-5 h-5 text-neutral-800 dark:text-neutral-200" />,
      title: {
        id: 'VIRTUAL CASTING',
        en: 'VIRTUAL CASTING',
        cn: '虚拟选角'
      },
      desc: {
        id: 'Sintesis model virtual berbasis etnisitas lokal demi relevansi pasar, dengan pose dinamis & ekspresi elegan terkalibrasi.',
        en: 'Demographically targeted virtual model synthesis to ensure market relevance, with calibrated pose and elegant expression.',
        cn: '针对人口统计学的虚拟模特合成，以确保市场相关性，并进行校准的姿势和优雅表情。'
      }
    },
    {
      icon: <Layers className="w-5 h-5 text-neutral-800 dark:text-neutral-200" />,
      title: {
        id: 'FIDELITY LOCK (PRESISI PRODUK)',
        en: 'FIDELITY LOCK (PRODUCT INTEGRITY)',
        cn: '物料保真锁（产品真实性）'
      },
      desc: {
        id: 'Menggunakan ControlNet berpemandu poligon untuk menyerap kain asli (nylon & jahitan fungsional) ke tubuh model AI tanpa distorsi visual.',
        en: 'Utilizes polygon-guided ControlNet pipelines to map physical fabrics (nylon weave & seam work) onto synthesized bodies with zero distortion.',
        cn: '利用先进的多边形引导 ControlNet 管道，将物理织物（尼龙编织和缝线工艺）映射到合成身体上，实现零失真。'
      }
    },
    {
      icon: <Sliders className="w-5 h-5 text-neutral-800 dark:text-neutral-200" />,
      title: {
        id: 'OBSIDIAN DUNES SYNTHESIS',
        en: 'OBSIDIAN DUNES SYNTHESIS',
        cn: '黑曜石沙丘合成'
      },
      desc: {
        id: 'Penciptaan latar belakang fiksi ilmiah berupa bukit pasir gelap obsidian, atmosfer kabut tipis, dan cincin planet raksasa bercahaya.',
        en: 'Custom 3D-guided background generation featuring obsidian desert dunes, atmospheric dust haze, and a giant glowing planetary ring.',
        cn: '定制的 3D 引导背景生成，具有黑曜石沙丘、大气沙尘雾霾以及巨大的发光行星环。'
      }
    },
    {
      icon: <TrendingUp className="w-5 h-5 text-neutral-800 dark:text-neutral-200" />,
      title: {
        id: 'EFISIENSI KOMERSIAL',
        en: 'COMMERCIAL EFFICIENCY',
        cn: '商业效率'
      },
      desc: {
        id: 'Memotong biaya lisensi model internasional, sewa studio fiksi ilmiah khusus, dan transportasi luar kota. Penghematan anggaran produksi fisik >85%.',
        en: 'Bypasses international model agency fees, physical sci-fi studio rentals, and location scouting. Physical production cost-saving of >85%.',
        cn: '绕过国际模特经纪公司费用、实体科幻摄影棚租金和外景考察。实体制作成本节省超过 85%。'
      }
    }
  ];

  return (
    <div id={`project-detail-${work.id}`} className="w-full min-h-screen bg-white dark:bg-[#121212] text-neutral-900 dark:text-neutral-100 transition-colors duration-300 pb-20 relative font-sans">
      
      {/* 2. MAIN NARRATIVE LAYOUT - FULL SEAMLESS ZCOOL INTEGRATION */}
      <div className="max-w-4xl mx-auto px-4 pt-8 pb-12 space-y-6">
        
        {/* Clean, Non-Sticky Back Link */}
        <button
          onClick={onBack}
          className="group inline-flex items-center gap-2 text-xs font-semibold tracking-wider uppercase text-neutral-400 hover:text-neutral-800 dark:text-neutral-500 dark:hover:text-neutral-200 transition-all duration-300 pb-2"
        >
          <ArrowLeft className="w-3.5 h-3.5 group-hover:-translate-x-0.5 transition-transform" />
          <span>{getLabel('back')}</span>
        </button>
        
        {/* ZCOOL STYLE MAIN TITLE ON TOP */}
        <div className="flex items-center gap-2.5 flex-wrap pt-2">
          <h1 
            className="text-2xl md:text-3xl lg:text-3.2xl font-extrabold tracking-tight text-neutral-900 dark:text-white leading-tight font-sans"
            title={`${detail.client} X ${work.id === 'w11' ? '草本初色' : 'AI Production'} - ${work.label}`}
          >
            {detail.client} {work.id === 'w11' ? 'X 草本初色' : ''} - {work.label}
          </h1>
          
          {/* ZCOOL fire icon indicator */}
          <div className="flex items-center justify-center w-7 h-7 rounded-full bg-red-50 dark:bg-red-950/40 text-red-500 hover:scale-115 transition-transform duration-300 cursor-pointer shadow-xs border border-red-100/50 dark:border-red-900/30 shrink-0" title="Homepage recommendation">
            <span className="text-base animate-pulse">🔥</span>
          </div>

          {/* Rank list gold badge */}
          <div className="flex items-center gap-1 bg-amber-500/10 text-amber-500 px-2.5 py-1 rounded-md text-[10px] font-mono font-bold tracking-widest uppercase border border-amber-500/20 shadow-xs shrink-0" title="Ranked Project">
            <span>🏅</span>
            <span>RANK</span>
          </div>
        </div>

        {/* ZCOOL-STYLE SEAMLESS CREATOR ROW */}
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pt-2 pb-6 border-b border-neutral-100 dark:border-neutral-800/60">
          
          {/* Creator Profile Details */}
          <div className="flex items-center gap-3">
            {/* Red Square Studio Logo - ZCOOL Agency Style */}
            <div className="w-11 h-11 rounded bg-red-600 dark:bg-red-700 flex flex-col items-center justify-center font-bold text-white leading-none shrink-0 border border-red-500 shadow-sm select-none">
              <span className="text-[10px] tracking-tighter font-extrabold font-sans">GUSTI</span>
              <span className="text-[5px] tracking-widest font-mono scale-90 mt-0.5">STUDIO</span>
            </div>

            <div className="space-y-0.5">
              <div className="flex items-center gap-1.5 flex-wrap">
                <h4 className="text-[13px] font-bold text-neutral-900 dark:text-neutral-100 font-sans tracking-tight">GUSTI. Visual Creative</h4>
                {/* ZCOOL Gold Badge */}
                <span className="w-3.5 h-3.5 rounded-full bg-amber-500 text-neutral-950 font-mono font-extrabold flex items-center justify-center text-[8.5px] shadow-xs" title="Verified Creator Pro">z</span>
                <span className="text-xs">🔥</span>
                {/* Agency Label */}
                <span className="text-[8px] font-sans font-extrabold tracking-widest bg-neutral-900 dark:bg-neutral-800 text-neutral-100 dark:text-neutral-200 px-1 py-0.5 rounded uppercase scale-90 origin-left">
                  {lang === 'cn' ? '机构' : 'PRO OPERATOR'}
                </span>
              </div>
              
              {/* Creator Metadata matching the requested ZCOOL screenshot */}
              <div className="flex flex-wrap items-center gap-x-2 text-[10.5px] text-neutral-400 dark:text-neutral-500 font-sans">
                <span>Guangzhou</span>
                <span className="text-neutral-300 dark:text-neutral-800 font-light">/</span>
                <span>{lang === 'id' ? 'Desainer Web' : lang === 'cn' ? '网页设计师' : 'Web Designer'}</span>
                <span className="text-neutral-300 dark:text-neutral-800 font-light">/</span>
                <span>{lang === 'id' ? '1 hari yang lalu' : lang === 'cn' ? '1天前发布' : '1 day ago'}</span>
                <span className="text-neutral-300 dark:text-neutral-800 font-light">/</span>
                <span className="flex items-center gap-0.5">
                  <Eye className="w-3 h-3 text-neutral-400 dark:text-neutral-500" />
                  <span>67,594 {lang === 'cn' ? '浏览' : 'views'}</span>
                </span>
                <span className="text-neutral-300 dark:text-neutral-800 font-light">/</span>
                <span className="text-[10px]">© {lang === 'id' ? 'Hak Cipta' : lang === 'cn' ? '版权所有' : 'Copyright'}</span>
              </div>
            </div>
          </div>

          {/* Action buttons exactly styled from the screenshot */}
          <div className="flex items-center gap-2 self-start md:self-auto">
            {/* Message Button */}
            <a
              href={getCustomWhatsAppLink()}
              target="_blank"
              referrerPolicy="no-referrer"
              className="px-4 py-1.5 border border-neutral-300 dark:border-neutral-700 hover:bg-neutral-50 dark:hover:bg-neutral-900 text-neutral-700 dark:text-neutral-300 rounded text-xs font-bold font-sans transition-all duration-300 flex items-center justify-center min-h-[30px]"
            >
              <span>{lang === 'id' ? 'Kirim Pesan' : lang === 'cn' ? 'Message' : 'Message'}</span>
            </a>

            {/* Split Follow Dropdown Button */}
            <div className="inline-flex rounded overflow-hidden shadow-xs shrink-0">
              <button
                onClick={() => setFollowed(!followed)}
                className={`px-4.5 py-1.5 text-xs font-extrabold transition-all duration-300 min-h-[30px] flex items-center gap-1.5 ${
                  followed 
                    ? 'bg-neutral-100 dark:bg-neutral-800 text-neutral-500 dark:text-neutral-400 hover:bg-neutral-200' 
                    : 'bg-[#ff5134] hover:bg-[#e0452a] text-white'
                }`}
              >
                {followed ? (
                  <span>{lang === 'id' ? 'Mengikuti' : lang === 'cn' ? '已关注' : 'Following'}</span>
                ) : (
                  <span>{lang === 'id' ? 'Ikuti' : lang === 'cn' ? 'Follow' : 'Follow'}</span>
                )}
              </button>
              
              {/* Dropdown Chevron component */}
              <button 
                className={`px-1.5 border-l min-h-[30px] flex items-center justify-center transition-all duration-300 ${
                  followed 
                    ? 'bg-neutral-100 dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700 text-neutral-500' 
                    : 'bg-[#ff5134] hover:bg-[#e0452a] border-[#ff6950] text-white'
                }`}
              >
                <span className="text-[8px] transform translate-y-px">▼</span>
              </button>
            </div>
          </div>
        </div>

        {/* SECTION 1: CAMPAIGN INFO GRID */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 py-6 border-b border-neutral-100 dark:border-neutral-800/60 text-sm">
          <div>
            <h4 className="text-[11px] font-extrabold tracking-wider text-neutral-400 dark:text-neutral-500 uppercase">{getLabel('client')}</h4>
            <p className="mt-1 font-bold text-neutral-800 dark:text-neutral-200">{detail.client}</p>
          </div>
          <div>
            <h4 className="text-[11px] font-extrabold tracking-wider text-neutral-400 dark:text-neutral-500 uppercase">{getLabel('year')}</h4>
            <p className="mt-1 font-bold text-neutral-800 dark:text-neutral-200">{detail.year}</p>
          </div>
          <div className="col-span-2">
            <h4 className="text-[11px] font-extrabold tracking-wider text-neutral-400 dark:text-neutral-500 uppercase">{getLabel('role')}</h4>
            <p className="mt-1 font-bold text-neutral-800 dark:text-neutral-200">{detail.role[lang] || detail.role['en']}</p>
          </div>
        </div>

        {/* SECTION 1B: APPLIED SPECIALTIES */}
        <div className="py-4 border-b border-neutral-100 dark:border-neutral-800/60">
          <h4 className="text-[11px] font-extrabold tracking-wider text-neutral-400 dark:text-neutral-500 uppercase mb-2">{getLabel('services')}</h4>
          <div className="flex flex-wrap gap-1.5">
            {(detail.services[lang] || detail.services['en'] || []).map((srv, idx) => (
              <span key={idx} className="text-[11px] font-semibold bg-neutral-100 dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300 px-3 py-1 rounded">
                {srv}
              </span>
            ))}
          </div>
        </div>

        {/* SECTION 2: THE STRATEGIC NARRATIVE */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 py-8 border-b border-neutral-100 dark:border-neutral-800/60 leading-relaxed text-[13px]">
          <div className="space-y-2">
            <div className="flex items-center gap-2 text-neutral-800 dark:text-neutral-200 font-extrabold tracking-tight">
              <span className="w-1.5 h-3 bg-red-500 rounded-sm"></span>
              <span>{getLabel('overview')}</span>
            </div>
            <p className="text-neutral-500 dark:text-neutral-400">
              {detail.content[lang]?.overview || detail.content['en']?.overview}
            </p>
          </div>

          <div className="space-y-2">
            <div className="flex items-center gap-2 text-neutral-800 dark:text-neutral-200 font-extrabold tracking-tight">
              <span className="w-1.5 h-3 bg-red-500 rounded-sm"></span>
              <span>{getLabel('direction')}</span>
            </div>
            <p className="text-neutral-500 dark:text-neutral-400 italic">
              "{detail.content[lang]?.creativeDirection || detail.content['en']?.creativeDirection}"
            </p>
          </div>

          <div className="space-y-2">
            <div className="flex items-center gap-2 text-neutral-800 dark:text-neutral-200 font-extrabold tracking-tight">
              <span className="w-1.5 h-3 bg-red-500 rounded-sm"></span>
              <span>{getLabel('scope')}</span>
            </div>
            <p className="text-neutral-500 dark:text-neutral-400">
              {detail.content[lang]?.scope || detail.content['en']?.scope}
            </p>
          </div>
        </div>

        {/* SECTION 3: INTERACTIVE BEFORE/AFTER SLIDER (EXCLUSIVE TO STARMAP) */}
        {work.id === 'w1' && (
          <div className="py-8 space-y-4 border-b border-neutral-100 dark:border-neutral-800/60">
            <div className="text-center space-y-1">
              <h3 className="text-sm font-extrabold tracking-widest text-neutral-900 dark:text-white uppercase">
                {starmapLabels.sliderTitle[lang] || starmapLabels.sliderTitle['en']}
              </h3>
              <p className="text-xs text-neutral-400 dark:text-neutral-500 max-w-xl mx-auto">
                {starmapLabels.sliderSub[lang] || starmapLabels.sliderSub['en']}
              </p>
            </div>

            {/* Interactive Slider Container */}
            <div className="relative w-full aspect-[4/3] rounded-2xl overflow-hidden shadow-lg border border-neutral-200 dark:border-neutral-800 bg-neutral-100 dark:bg-neutral-900 select-none">
              
              {/* Before Image (Bottom) - Using raw outerwear flatlay */}
              <img 
                src="https://images.unsplash.com/photo-1591047139829-d91aecb6caea?w=1200&q=90" 
                alt="Before Flatlay" 
                className="absolute inset-0 w-full h-full object-cover"
                draggable="false"
              />
              <div className="absolute top-4 left-4 bg-black/70 text-white text-[10px] font-mono font-extrabold px-2.5 py-1 rounded tracking-wider uppercase z-10">
                {starmapLabels.beforeLabel[lang] || starmapLabels.beforeLabel['en']}
              </div>

              {/* After Image (Top, Clipped) - Utilizing the beautiful /starmap.jpg */}
              <div 
                className="absolute inset-0 pointer-events-none"
                style={{ clipPath: `polygon(0 0, ${sliderValue}% 0, ${sliderValue}% 100%, 0 100%)` }}
              >
                <img 
                  src="/starmap.jpg" 
                  alt="After Creative Production" 
                  className="absolute inset-0 w-full h-full object-cover animate-fade-in"
                  draggable="false"
                />
                <div className="absolute top-4 right-4 bg-red-600 text-white text-[10px] font-mono font-extrabold px-2.5 py-1 rounded tracking-wider uppercase z-10">
                  {starmapLabels.afterLabel[lang] || starmapLabels.afterLabel['en']}
                </div>
              </div>

              {/* Slider Line Overlay */}
              <div 
                className="absolute top-0 bottom-0 w-1 bg-white cursor-ew-resize z-20 pointer-events-none"
                style={{ left: `${sliderValue}%` }}
              >
                {/* Handle icon */}
                <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-white dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-700 shadow-md flex items-center justify-center">
                  <Sliders className="w-4 h-4 text-neutral-800 dark:text-neutral-200" />
                </div>
              </div>

              {/* HTML Range Input for Interaction */}
              <input 
                type="range" 
                min="0" 
                max="100" 
                value={sliderValue} 
                onChange={(e) => setSliderValue(Number(e.target.value))}
                className="absolute inset-0 w-full h-full opacity-0 cursor-ew-resize z-30"
              />

            </div>
          </div>
        )}

        {/* SECTION 4: STARMAP BLUEPRINT SPECIFICATION */}
        {work.id === 'w1' && (
          <div className="py-8 space-y-6 border-b border-neutral-100 dark:border-neutral-800/60">
            <div className="text-center space-y-1">
              <h3 className="text-sm font-extrabold tracking-widest text-neutral-900 dark:text-white uppercase flex items-center justify-center gap-1.5">
                <Sparkles className="w-4 h-4 text-amber-500 animate-pulse" />
                <span>{starmapLabels.specTitle[lang] || starmapLabels.specTitle['en']}</span>
              </h3>
              <p className="text-xs text-neutral-400 dark:text-neutral-500 max-w-xl mx-auto">
                {starmapLabels.specSub[lang] || starmapLabels.specSub['en']}
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {starmapSpecs.map((spec, sidx) => (
                <div 
                  key={sidx}
                  className="bg-[#F5F6F8] dark:bg-[#161616] border border-neutral-200/50 dark:border-neutral-800/50 p-5 rounded-2xl hover:border-neutral-300 dark:hover:border-neutral-700 transition-all duration-300 flex items-start gap-4"
                >
                  <div className="p-3 bg-white dark:bg-neutral-800 rounded-xl shadow-xs border border-neutral-200/30 dark:border-neutral-700/30 shrink-0">
                    {spec.icon}
                  </div>
                  <div className="space-y-1.5">
                    <h4 className="text-[12.5px] font-extrabold text-neutral-800 dark:text-neutral-200 tracking-tight">
                      {spec.title[lang] || spec.title['en']}
                    </h4>
                    <p className="text-xs text-neutral-500 dark:text-neutral-400 leading-relaxed">
                      {spec.desc[lang] || spec.desc['en']}
                    </p>
                  </div>
                </div>
              ))}
            </div>
          </div>
        )}

        {/* SECTION 5: LONG VERTICAL MEDIA GALLERY */}
        <div className="py-6 space-y-6">
          {/* Main primary rendering (Cover Image / Cinematic Video if available) */}
          {detail.videoUrl ? (
            <div className="relative w-full aspect-video rounded-2xl overflow-hidden shadow-lg bg-black border border-neutral-200 dark:border-neutral-800">
              <video
                src={detail.videoUrl}
                loop
                muted={videoMuted}
                autoPlay={videoPlaying}
                className="w-full h-full object-cover"
                playsInline
              />
              <div className="absolute bottom-4 right-4 flex items-center gap-2 z-10">
                <button
                  onClick={() => setVideoPlaying(!videoPlaying)}
                  className="p-2 bg-black/60 hover:bg-black/80 text-white rounded-full transition-colors backdrop-blur-xs"
                >
                  {videoPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
                </button>
                <button
                  onClick={() => setVideoMuted(!videoMuted)}
                  className="p-2 bg-black/60 hover:bg-black/80 text-white rounded-full transition-colors backdrop-blur-xs"
                >
                  {videoMuted ? <VolumeX className="w-4 h-4" /> : <Volume2 className="w-4 h-4" />}
                </button>
              </div>
            </div>
          ) : (
            /* Cover Image if no video */
            <div className="w-full aspect-[4/3] md:aspect-video rounded-2xl overflow-hidden shadow-md border border-neutral-200 dark:border-neutral-800 bg-neutral-100 dark:bg-neutral-900">
              <img 
                src={work.image || detail.images[0]} 
                alt={detail.client} 
                className="w-full h-full object-cover"
              />
            </div>
          )}

          {/* Vertical Stream of Assets */}
          <div className="space-y-6">
            {/* Render detailed mediaList (specifically configured for high-end assets e.g., w11) */}
            {detail.mediaList && detail.mediaList.length > 0 ? (
              detail.mediaList.map((media, midx) => (
                <div key={midx} className="space-y-3">
                  <div className={`w-full rounded-2xl overflow-hidden border border-neutral-200 dark:border-neutral-800 shadow-sm bg-neutral-100 dark:bg-neutral-900 ${media.aspectRatio || 'aspect-video'}`}>
                    {media.type === 'video' ? (
                      <video src={media.url} controls muted loop playsInline className="w-full h-full object-cover" />
                    ) : (
                      <img src={media.url} alt={media.caption?.[lang] || ''} className="w-full h-full object-cover" />
                    )}
                  </div>
                  {(media.caption || media.subText) && (
                    <div className="px-2 text-center space-y-1 py-1 max-w-2xl mx-auto">
                      {media.caption && (
                        <h5 className="text-xs font-bold text-neutral-800 dark:text-neutral-200 tracking-tight uppercase">
                          {media.caption[lang] || media.caption['en']}
                        </h5>
                      )}
                      {media.subText && (
                        <p className="text-[11px] text-neutral-400 dark:text-neutral-500 leading-relaxed">
                          {media.subText[lang] || media.subText['en']}
                        </p>
                      )}
                    </div>
                  )}
                </div>
              ))
            ) : (
              /* Fallback simple sequential flow of high-resolution images */
              detail.images.map((img, idx) => (
                <div key={idx} className="w-full rounded-2xl overflow-hidden border border-neutral-200 dark:border-neutral-800 shadow-sm bg-neutral-100 dark:bg-neutral-900">
                  <img 
                    src={img} 
                    alt={`${detail.client} Asset ${idx + 1}`} 
                    className="w-full object-cover" 
                    loading="lazy"
                  />
                </div>
              ))
            )}
          </div>
        </div>

        {/* SECTION 6: CREDITS ROLL (For projects with structured credits list e.g. w11) */}
        {detail.credits && detail.credits.length > 0 && (
          <div className="py-8 border-t border-b border-neutral-100 dark:border-neutral-800/60 bg-[#F5F6F8]/50 dark:bg-[#161616]/30 px-6 rounded-2xl space-y-5">
            <h4 className="text-xs font-mono font-bold tracking-widest text-neutral-400 dark:text-neutral-500 uppercase text-center">
              —— {lang === 'id' ? 'KREDIT TIM PRODUKSI' : lang === 'cn' ? '项目制作群' : 'PRODUCTION CREDITS'} ——
            </h4>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-y-4 gap-x-6 text-[11.5px] font-sans">
              {detail.credits.map((credit, cidx) => (
                <div key={cidx} className="flex flex-col gap-0.5">
                  <span className="text-neutral-400 dark:text-neutral-500 font-medium">
                    {credit.role[lang] || credit.role['en']}
                  </span>
                  <span className="text-neutral-800 dark:text-neutral-200 font-extrabold">
                    {credit.name}
                  </span>
                </div>
              ))}
            </div>
          </div>
        )}

        {/* SECTION 7: HIGH-CONVERTING B2B CTA */}
        <div className="pt-10 pb-4 text-center">
          <div className="bg-[#1A1A1A] dark:bg-[#1A1A1A] text-white p-8 md:p-10 rounded-[28px] border border-neutral-800/80 shadow-2xl relative overflow-hidden flex flex-col items-center">
            
            {/* Ambient Background Glow Accent */}
            <div className="absolute -top-16 -right-16 w-36 h-36 rounded-full bg-red-600/10 blur-[60px]" />
            <div className="absolute -bottom-16 -left-16 w-36 h-36 rounded-full bg-red-500/10 blur-[60px]" />

            <div className="relative z-10 space-y-4 max-w-xl">
              <div className="inline-flex items-center gap-1.5 bg-red-600/10 text-[#FF5134] px-3 py-1 rounded-full text-[10px] font-mono font-extrabold tracking-widest uppercase border border-red-500/20 shadow-xs mb-1">
                <span>⚡</span>
                <span>{lang === 'id' ? 'KONSULTASI GRATIS' : lang === 'cn' ? '免费视觉咨询' : 'FREE DIRECT SCOPING'}</span>
              </div>
              
              <h3 className="text-xl md:text-2xl font-extrabold tracking-tight font-sans text-white uppercase">
                {getLabel('interest')}
              </h3>
              
              <p className="text-xs md:text-[13px] text-neutral-400 leading-relaxed font-sans font-medium">
                {lang === 'id' 
                  ? 'Gusti siap merancang visual produk fisik / aset komersial beralur tinggi yang disesuaikan khusus untuk target pasar Anda. Hubungi kami sekarang demi mendapatkan estimasi produksi.'
                  : lang === 'cn'
                  ? 'Gusti 准备好为您的产品研发、商业详情页提供高端视觉支持，定制高度吸引眼球的商业素材。现在联系我们以获取方案估算。'
                  : 'Gusti is ready to curate and produce premium high-converting visual assets tailor-made for your target market. Contact us directly to draft a production quotation.'}
              </p>

              <div className="pt-4 flex justify-center">
                <a
                  href={getCustomWhatsAppLink()}
                  target="_blank"
                  referrerPolicy="no-referrer"
                  className="px-6 md:px-8 py-3.5 bg-[#ff5134] hover:bg-[#e0452a] text-white font-extrabold text-xs tracking-wider uppercase rounded-xl transition-all duration-300 shadow-lg flex items-center justify-center gap-2 group transform active:scale-95"
                >
                  <MessageCircle className="w-4 h-4" />
                  <span>{getLabel('chat_gusti')}</span>
                  <ChevronRight className="w-3.5 h-3.5 group-hover:translate-x-0.5 transition-transform" />
                </a>
              </div>
            </div>
          </div>
        </div>

      </div>

    </div>
  );
}
