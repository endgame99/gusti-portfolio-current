import { categories, creatorVideoPath, creatorVideos, serviceTemplates, servicePages, type Aspect, type ServiceTemplate } from '../content/services/templates';
...
export function ServicesPage({ searchQuery }: ServicesProps) {
  ...
  return (
    <div className="py-6">
      ...
      {!searchQuery && (
        <nav aria-label="Service galleries" className="mb-6 flex flex-wrap gap-3">
          {servicePages.map(page => (
            <a key={page.href} href={page.href} className="rounded-full border border-neutral-200 px-4 py-2 type-label-md transition-colors hover:bg-neutral-100 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 dark:border-neutral-700 dark:hover:bg-neutral-800">{page.title}</a>
          ))}
        </nav>
      )}
